...
- Schema fallback tests use the `pipestream-wiremock-server` container (0.1.56+).
- CI runners must have Docker available for Testcontainers.
- Override the WireMock image if needed:
  - Environment: `PIPESTREAM_WIREMOCK_IMAGE=docker.io/pipestreamai/pipestream-wiremock-server:0.1.56`
  - System property: `-Dpipestream.wiremock.image=docker.io/pipestreamai/pipestream-wiremock-server:0.1.56`

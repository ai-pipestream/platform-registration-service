 ...
 */
@ApplicationScoped
public class DynamicGrpcClientFactory implements ai.pipestream.common.grpc.GrpcClientFactory {
    ...
    public Uni<Channel> getChannel(String serviceName) {
        ...
        return serviceDiscoveryManager.ensureServiceDefined(serviceName)
                ...
                .chain(instances -> {
                    LOG.infof("Step 2: Got %s instances for %s", instances.size(), serviceName);
                    try {
                        LOG.debugf("Step 2a: instances list type=%s, elementType=%s",
                                instances.getClass().getName(),
                                instances.isEmpty() ? "<empty>" : instances.get(0).getClass().getName());
                    } catch (Exception e) {
                        LOG.debugf("Step 2a: Unable to inspect instances list: %s", e.getMessage());
                    }
                    Uni<Channel> channelUni = channelManager.getOrCreateChannel(serviceName, instances);
                    LOG.infof("Step 2b: channelUni type: %s", channelUni.getClass().getName());
                    return channelUni;
                });
    }
    ...
    // Manual eviction method for when a service is known to be gone
    ...
    public static class StorkNameResolver extends NameResolver {
        ...
        private void resolve() {
            try {
                ...
                LOG.errorf(e, "Failed to resolve service instances for %s", serviceName);

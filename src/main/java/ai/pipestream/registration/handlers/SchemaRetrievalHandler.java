...
public class SchemaRetrievalHandler {
     ...
     */
    private Uni<GetModuleSchemaResponse> getSchemaFromModule(String serviceName) {
        LOG.infof("Falling back to direct module call for schema: %s", serviceName);

        return grpcClients.getPipeStepProcessorClient(serviceName)
            .flatMap(stub ->
    ...
    /**
     * Build response from module's GetServiceRegistrationResponse
     */
    private GetModuleSchemaResponse buildResponseFromModuleMetadata(
            String serviceName,
            GetServiceRegistrationResponse metadata) {
    ...
    /**
     * Get all available schema versions for a module
     */
        ...
        return moduleRepository.findSchemaVersionsByServiceName(serviceName)
            .map(schemas -> {
                ...
                for (ConfigSchema schema : schemas) {
                    ...
                    // Track latest version and artifact ID
            ...
            .onItem().ifNull().switchTo(() -> {
                // If no schemas in database, return empty response
                return Uni.createFrom().item(GetModuleSchemaVersionsResponse.newBuilder()
                    .setModuleName(serviceName)
                    .build());
            });
    }
